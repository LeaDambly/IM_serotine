...
MakeIntegrationStack <- function(mesh, data, area, tag='mesh',
  ...
  nv <- mesh$n # number of nodes in our mesh
  projmat.ip <- Diagonal(nv, rep(1, nv))
  
  stk.ip <- inla.stack(data = list(resp = cbind(rep(0,nv), NA), e = area),
                       A = list(1,projmat.ip), tag = tag,
                       effects = list(NearestCovs@data, list(i = 1:nv)))
  return(stk.ip)
}